...
public type GamesPlayed = Types.GamesPlayed;
...
public func player_state_to_view(player: PlayerState): PlayerView {
  { name = player.name; score = player.score; games_played = player.games_played; }
};
...
public func update_top_players(top_players: [var ?PlayerView], name_: PlayerName, score_: Score, games_played_: GamesPlayed) {
  ...
  for (i in Iter.range(0, N - 1)) {
    switch (top_players[i]) {
      case null {
        top_players[i] := ?{ name = name_; score = score_; games_played = games_played_ };
        return;
      };
      case (?player) {
...
public func init_top_players(players: Iter<PlayerState>) : [var ?PlayerView] {
   ...
   Iter.iterate<PlayerState>(players, func(player, _) {
     update_top_players(top_players, player.name, player.score, player.games_played)
   });
   top_players
};
